...
export const CategoryDropdown = ({ category, isActive, isNavigationHovered }: CategoryDropdownProps) => {
  ...
  return (
    <div className="relative" onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave} ref={dropdownRef} onClick={toggleDropdown}>
      <div className="relative">
        <Button variant={"elevated"} className={cn(
          ...
          <Link href={`/${category.slug === "all" ? "" : category.slug}`} className="flex items-center gap-2 w-full h-full">
            {category.name}
          </Link>
        </Button>
